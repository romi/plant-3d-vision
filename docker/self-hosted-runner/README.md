...
2. [Host setup](#host-setup)
    ...
    - [Configure NVIDIA runtime](#configure-nvidia-runtime)
...
4. [Running the runner](#running-the-runner)
    - [Create an `.env` file](#env-file)
    - [Build & start](#build‑and‑start)
    - [View logs / stop / rebuild](#logs‑stop‑rebuild)
...
** (or your preferred shell rc file) so every new session knows where to find the rootless Docker socket:
...
The script runs when the container starts and performs four checks:
...
The compose file defines a single service named `runner`:
...
Only the values inside the file are read by the container; the file itself is **not** baked into the Docker image.

```
dotenv
# ---------- GitHub ----------
GITHUB_RUNNER_URL=https://github.com/<YOUR_USERNAME>/<YOUR_REPOSITORY>
GITHUB_RUNNER_TOKEN=YOUR_REGISTRATION_TOKEN   # get from the repo > Settings > Actions > Runners
...
# ---------- Docker ----------
DOCKER_SOCK=/run/user/$(id -u)/docker.sock   # Path to the rootless Docker socket
```

> **Security note:** Treat this file like a secret.
...
### 5. Rebuild after making changes (e.g., Dockerfile updates)

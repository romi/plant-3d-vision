...
class AnglesAndInternodesEvaluation(EvaluationTask):
    ...
    def run(self):
        ...
        if str(self.ground_truth.task_family) == "VirtualPlantObj":
            # retrieve the angles and inter-nodes ground truth from generated plants metadata
            ...
            # retrieve the angles and inter-nodes ground truth from manual measures (measures.json)
            input_file = self.input_file()
        ...
        # Creates the ground-truth array of angles and inter-nodes:
        seq_gt = np.array([angles_gt, internodes_gt]).T
        # Creates the predicted array of angles and inter-nodes:
        ...
        dtwcomputer.plot_results(f_path)

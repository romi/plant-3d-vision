...
def get_sequences_data(db_path, dataset_name):
    ...
    # Try to load manual measurements:
    gt_measures = scan.get_measures()
    if gt_measures != {}:
        logger.info(f"Found ground-truth measurements.")
    if gt_measures != {} and is_radians(gt_measures['angles']):
    # Convert the GT angles to degrees if in radians:
        gt_measures['angles'] = list(map(degrees, gt_measures["angles"]))
        logger.info("Converted ground-truth angles from radians to degrees.")

    # Find the fileset corresponding to the task:
    fileset_names = locate_task_filesets(scan, TASKS)
    # Load the measurements from the "AnglesAndInternodes" fileset:
    measures_fs = scan.get_fileset(fileset_names['AnglesAndInternodes'])
    measures_file = measures_fs.get_file('AnglesAndInternodes')
    auto_measures = read_json(measures_file)
    try:
        assert auto_measures != {}
    except:
        auto_measures = {"angles": [np.nan], "internodes": [np.nan]}
        logger.warning(f"No AnglesAndInternodes sequences found!")
    else:
        angles = auto_measures["angles"]
        internodes = auto_measures["internodes"]
        ...
        if is_radians(auto_measures["angles"]):
            auto_measures["angles"] = list(map(degrees, auto_measures["angles"]))
    ...
    # Add the ground-truth measures, if any:
    if gt_measures != {}:
        measures['gt_angles'] = gt_measures["angles"]
        measures['gt_internodes'] = gt_measures["internodes"]
...
if __name__ == '__main__':
    ...
    logger = get_logger('dash_explorer')
